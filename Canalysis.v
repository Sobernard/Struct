...
Lemma norm_Re (x : complexR) : (norm (Re_R x)) <= (norm x).
Proof.
...
Lemma norm_Im (x : complexR) : (norm (Im_R x)) <= (norm x).
Proof.
rewrite /norm /=; case x => xr xi /=.
move: (sqrt_plus_sqr (Rabs xr) (Rabs xi)).
rewrite !RPow_abs !exp_R !Rabs_Rabsolu.
...
Lemma norm_ReIm (x : complexR) :
    (norm x) <= (norm (Re_R x)) + (norm (Im_R x)).
Proof.
rewrite /norm /=; case x => xr xi /=.
...
Definition is_Crlim_C (f : R -> complexR) (z : R ) (l : complexR) :=
  filterlim (fun x => Re_R (f x)) (locally' z) (locally (Re_R l)) /\
  filterlim (fun x => Im_R (f x)) (locally' z) (locally (Im_R l)).
...
Definition Cr_limseq (u : nat -> complexR) : complexR :=
  RtoC (real (Lim_seq (fun n => Re_R (u n)))) + 'i *
  RtoC (real (Lim_seq (fun n => Im_R (u n)))).

Definition Cr_lim (f : R -> complexR) (z : R) : complexR :=
  RtoC (real (Lim (fun x => Re_R (f x)) z)) + 'i *
  RtoC (real (Lim (fun x => Im_R (f x)) z)).

Lemma is_Crlim_C_unique (f : R -> complexR) (z : R) (l : complexR) :
  is_Crlim_C f z l -> Cr_lim f z = l.
Proof.
rewrite /is_Crlim_C /Cr_lim; move=> [Hre Him].
have -> : Lim (fun x : R => Re_R (f x)) z = Re_R l by apply: is_lim_unique.
have -> : Lim (fun x : R => Im_R (f x)) z = Im_R l by apply: is_lim_unique.
...
Lemma Crcontinuity_pt_ReIm (f : R -> complexR) (x : R) :
  Crcontinuity_pt f x ->
    continuity_pt (fun y => Re_R (f y)) x /\
    continuity_pt (fun y => Im_R (f y)) x.
...
apply: Crcontinuity_pt_R; split.
  apply: (continuity_pt_ext (plus_fct (fun y => Re_R (f y)) (fun y => Re_R (g y)))).
    by move=> u; rewrite /plus_fct Rplus_add raddfD /=.
  by apply: continuity_pt_plus.
apply: (continuity_pt_ext (plus_fct (fun y => Im_R (f y)) (fun y => Im_R (g y)))).
  by move=> u; rewrite /plus_fct Rplus_add raddfD.
by apply: continuity_pt_plus.
Qed.
...
apply: Crcontinuity_pt_R; split.
  apply: (continuity_pt_ext (opp_fct (fun y => Re_R (f y)))).
    by move=> u; rewrite raddfN.
  by apply: continuity_pt_opp.
apply: (continuity_pt_ext (opp_fct (fun y => Im_R (f y)))).
  by move=> u; rewrite raddfN.
by apply: continuity_pt_opp.
Qed.
...
apply: (@Crcontinuity_pt_ext (fun y =>   
    ((RtoC (Re_R (f y) * Re_R (g y)) + 'i * RtoC (Re_R (f y) * Im_R (g y))) +
    (RtoC ((- Im_R (f y)) * Im_R (g y)) + 'i * RtoC (Im_R (f y) * Re_R (g y)))))).
...
apply: (continuity_pt_ext (mult_fct
      (opp_fct (fun y => Im_R (f y))) (fun y => Im_R (g y)))) => //.
...
apply: (@Crcontinuity_pt_ext (fun y => (RtoC (exp ((Re_R a) * y))) 
  ...
  by rewrite [_ * Im_R _]mulrC [_ * Re_R _]mulrC.
...
Definition is_Crderive_C (f : R -> complexR) (z : R) (l : complexR) :=
  is_derive (fun x => Re_R (f x)) z (Re_R l) /\
  is_derive (fun x => Im_R (f x)) z (Im_R l).

Lemma is_Crderive_C_eq (f : R_NormedModule -> Cr_R_NormedModule) z l :
  is_derive f z l <-> is_Crderive_C f z l.
Proof.
have Hr_Re x y : (minus (minus (Re_R (f y)) (Re_R (f x))) (scal (minus y x) (Re_R l))) 
  = Re_R ( (minus (minus (f y) (f x)) (scal (minus y x) l) ) ).    
  rewrite /minus /plus /opp /= !raddfB /= !Rplus_add !Ropp_opp.
  by congr (_ - _ - _); rewrite /scal /= linearZ.
have Hr_Im x y : (minus (minus (Im_R (f y)) (Im_R (f x))) (scal (minus y x) (Im_R l))) 
  = Im_R ( (minus (minus (f y) (f x)) (scal (minus y x) l) ) ).    
  rewrite /minus /plus /opp /= !raddfB /= !Rplus_add !Ropp_opp.
  by congr (_ - _ - _); rewrite /scal /= linearZ.
...
Lemma ex_Crderive_ReIm (f : R_NormedModule -> Cr_R_NormedModule) z :
  ex_derive f z ->
    ex_derive (fun x => Re_R (f x)) z /\
    ex_derive (fun x => Im_R (f x)) z.
Proof.
...
Definition Crderive_C (f : R -> complexR) (z : R) : complexR :=
  RtoC (Derive (fun x => Re_R (f x)) z) + 'i *
  RtoC (Derive (fun x => Im_R (f x)) z).
...
apply/eqP; rewrite /Derive.
  rewrite (@Lim_bis _ (fun h : R => 
     Re_R (    (f (z + h) - (f z) ) / (RtoC (z + h - z))  ))  (Finite 0)).
...
rewrite (@Lim_bis _ (fun h : R => 
     Im_R (    (f (z + h) - (f z) ) / (RtoC (z + h - z))  ))  (Finite 0)).
...
apply/andP; split; apply/eqP.
  move: (Lim_ext   (fun x0 : R => Re_R ((g x0 - g x) / RtoC (x0 - x)))
  (fun x0 : R => Re_R ((f x0 - f x) / RtoC (x0 - x))) x) => H.
  by rewrite H //; move=> y; rewrite !Heq.
move: (Lim_ext   (fun x0 : R => Im_R ((g x0 - g x) / RtoC (x0 - x)))
  (fun x0 : R => Im_R ((f x0 - f x) / RtoC (x0 - x))) x) => H.
by rewrite H //; move=> y; rewrite !Heq.
Qed.
...
rewrite -!Cr_scalE -scalerAl !linearZ /= -['i * _]mulrC ImiRe ReiNIm; split.
  apply: (is_derive_ext (fun x : R_NormedModule => (Rmult (Re_R k) (Re_R (f x))) -
       (Rmult (Im_R k) (Im_R (f x))) : R_NormedModule)).
    move=> t; rewrite [(f t)]complexE /= mulrDl !mulr0 !mul0r !subr0 !add0r.
    by rewrite !mul1r !mulr1 !addr0 !Rmult_mul.
  by rewrite mulrN -Rmult_mul; apply/is_derive_minus; apply/is_derive_scal.
apply: (is_derive_ext (fun x : R_NormedModule => (Rmult (Re_R k) (Im_R (f x))) +
       (Rmult (Im_R k) (Re_R (f x))) : R_NormedModule)).
  move=> t; rewrite [(f t)]complexE /= mulrDl !mulr0 !mul0r !subr0 !add0r.
  by rewrite !mul1r !mulr1 !addr0 !Rmult_mul.
by rewrite -Rmult_mul; apply/is_derive_plus; apply/is_derive_scal.
Qed.
...
rewrite ![(Im_R lg) * _]mulrC [(Re_R lg)* _]mulrC ; split.
  apply: (is_derive_ext (fun x : R_NormedModule => (Rmult (Re_R (f x)) (Re_R (g x))) 
     - (Rmult (Im_R (f x)) (Im_R (g x)))  : R_NormedModule)).
    move=> t; rewrite [(f t)]complexE /= mulrDl !mulr0 !mul0r !subr0 !add0r.
    rewrite !mul1r !addr0 !Rmult_mul /= ['i * _]mulrC -!Cr_scalE raddfD /=.
    by rewrite -scalerAl !linearZ /= ['i * _]mulrC ReiNIm mulrN.
  rewrite addrACA -opprD -!Rmult_mul -Rplus_add.
  by apply/is_derive_minus; apply/Derive.is_derive_mult.
apply: (is_derive_ext (fun x : R_NormedModule => (Rmult (Re_R (f x)) (Im_R (g x))) 
     + (Rmult (Im_R (f x)) (Re_R (g x)))  : R_NormedModule)).
  move=> t; rewrite [(f t)]complexE /= mulrDl !mulr0 !mul0r !subr0 !add0r.
  rewrite !mul1r !addr0 !Rmult_mul /= ['i * _]mulrC -!Cr_scalE raddfD /=.
  by rewrite -scalerAl !linearZ /= ['i * _]mulrC ImiRe.
rewrite addrAC !addrA -addrA addrACA -!Rmult_mul -Rplus_add.
apply/is_derive_plus; first by apply/Derive.is_derive_mult.
rewrite Rmult_mul mulrC -Rmult_mul addrC -Rplus_add.
...
rewrite -(Derive_Reals exp _ (derivable_pt_exp (Re_R a * x))).
...
rewrite Derive_mult; first last.
  ...
  by apply: ex_derive_const.
rewrite Derive_const Derive_id.
rewrite -(Derive_Reals cos _ (derivable_pt_cos (Im_R a * x))).
...
have -> : (- S1 + 'i * C1) = 'i * (C1 + 'i * S1).
  by rewrite mulrDr mulrA -{4}['i]expr1 -exprS sqrCi addrC mulNr mul1r.
set D := (C1 + _); set E := exp _.
rewrite scalerA mulrC -scalerA scalerAr; set F := E *: _.
by rewrite !Cr_scalE -mulrCA [X in _ = X * _]complexE /= mulrDl mulrA.
Qed.
...
Lemma Riemann_sum_Re (f : R -> Cr_R_NormedModule) (ptd : SF_seq) :
  Re_R (Riemann_sum f ptd) = Riemann_sum (fun x => Re_R (f x)) ptd.
Proof.
apply (SF_cons_ind (fun y => 
   Re_R (Riemann_sum f y) = Riemann_sum (fun x : R => Re_R (f x)) y) )
     => [x0 //| [x1 y0] y IH].
by rewrite !Riemann_sum_cons -IH raddfD /scal /= linearZ /=.
Qed.

Lemma Riemann_sum_Im (f : R -> Cr_R_NormedModule) (ptd : SF_seq) :
  Im_R (Riemann_sum f ptd) = Riemann_sum (fun x => Im_R (f x)) ptd.
apply (SF_cons_ind (fun y => 
   Im_R (Riemann_sum f y) = Riemann_sum (fun x : R => Im_R (f x)) y) )
     => [x0 //| [x1 y0] y IH].
by rewrite !Riemann_sum_cons -IH raddfD /scal /= linearZ.
Qed.
...
Lemma Riemann_sum_ReIm (f : R -> Cr_R_NormedModule) (ptd : SF_seq) :
  Riemann_sum f ptd =
  RtoC (Riemann_sum (fun x => Re_R (f x)) ptd) + 'i *
  RtoC (Riemann_sum (fun x => Im_R (f x)) ptd).
...
Definition is_CrInt (f : R -> complexR) (a b : R) (If : complexR):=
  (is_RInt (fun x => Re_R (f x)) a b (Re_R If)) /\
  (is_RInt (fun x => Im_R (f x)) a b (Im_R If)).
...
split.
  rewrite /is_CrInt; move=> Hint; split; move=> P [eP HP].
    move: (Hint (fun x => P (Re_R x))) => [| ef Hf'].
  ...
  move: (Hint (fun x => P (Im_R x))) => [| ef Hf'].
    exists eP => y; rewrite /ball /= /Cr_ball.
    by move=> [Br Bi]; apply: HP.
...
Lemma ex_RInt_C_eq (f : R -> Cr_R_NormedModule) a b :
  ex_RInt f a b <->
  ex_RInt (fun y => Re_R (f y)) a b /\ 
  ex_RInt (fun y => Im_R (f y)) a b.
Proof.
split.
...
Proof. by rewrite is_CrInt_C_eq CrInt_C_eq; apply: CrInt_C_unique. Qed.
...
Proof.
move=> ex_H Hcont; rewrite /Crderive_C /CrInt_C; simpc.
rewrite (RInt_ext _ (Derive (fun y => Re_R ( f y)))); last first.
  by move=> x Hx /=; simpc.
rewrite [X in _ * RtoC X](RInt_ext _ (Derive (fun y => Im_R ( f y)))); last first.
  by move=> x Hx /=; simpc.
apply/eqP; rewrite eq_complex /=; simpc.
...
rewrite (RInt_ext  (fun x : R => Re_R (f x)) (fun x : R => Re_R (g x))).
  rewrite (RInt_ext  (fun x : R => Im_R (f x)) (fun x : R => Im_R (g x))) //.
...
have Hcontr : (forall x : R, and (Rle (Rmin a b) x) (Rle x (Rmax a b))
   -> continuity_pt  (fun x : R => Re_R (f x)) x ).
...
have Hconti : (forall x : R, and (Rle (Rmin a b) x) (Rle x (Rmax a b))
   -> continuity_pt  (fun x : R => Im_R (f x)) x ).
  move=> x [/RleP Hmin /RleP Hmax].
  have Hineq: Rmin a b <= x <= Rmax a b.
    by apply/andP; split.
...
have Hci : (forall x, Rmin a b <= x <= Rmax a b -> continuity_pt 
  ...
  by move/Crcontinuity_pt_ReIm: (H x Hineq) => [_].
apply: (ex_RInt_ext
          (fun y => sqrt (Rplus (Rmult (Re_R (f y)) (Re_R (f y)))
                            (Rmult (Im_R (f y)) (Im_R (f y)))))).
